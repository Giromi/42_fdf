...
/*   Updated: 2022/07/25 01:45:14 by minsuki2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(void)
{
	...
	int img_width = HALF_WIDTH;
	int img_height = HALF_HEIGHT;
	int **map = {0000}
	int img_height = HALF_HEIGHT;
	...
	for (int i = 0 ; i < 100 ; i++)
	{
		...
		rot_z(&x, &y, img_width / 2, img_height / 2, -M_PI / 4);
	...
	for (int i = 0 ; i < 100 ; i++)
		my_mlx_pixel_put(&image, img_width / 2 + i, img_height / 2, 0x00FFFFFF);
	for (int i = 0 ; i < 100 ; i++)
		my_mlx_pixel_put(&image, img_width / 2, img_height / 2 - i, 0x00FFFFFF);
